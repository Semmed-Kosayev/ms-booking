...
public class BookingServiceAspect {
    ...
    @Around("execution(* az.edu.turing.booking.service.BookingService.*(..))")
    ...
    @Before("execution(* az.edu.turing.booking.service.BookingService.*(..))")
    public void logBeforeMethod(JoinPoint joinPoint) {
        log.info("Method called: {}", joinPoint.getSignature().getName());
    }

    @AfterReturning(value = "execution(* az.edu.turing.booking.service.BookingService.*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        log.info("Method successfully completed: {} | Result: {}", joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(value = "execution(* az.edu.turing.booking.service.BookingService.*(..))", throwing = "ex")
